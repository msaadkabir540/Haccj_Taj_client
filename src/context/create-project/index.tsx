...
import { ContextValueObjectInterface } from "./context-interface";
...
export const ContextAPI: React.FC<ContextAPIProps> = ({ children }: ContextAPIProps) => {
  ...
  const currentUser = { name: loggedInUser?.username, userId: loggedInUser?._id };
    ...
    try {
      ...
      if (response) {
        ...
        if (draftVideoIndex !== -1) {
          const updatedVideoDrafts = [...videoDrafts];
